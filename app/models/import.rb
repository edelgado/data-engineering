  ...
  def process_file
    ...
    CSV.foreach(self.file.path, {col_sep: "\t", headers: :first_row, return_headers: false}) do |row|
      ...
      a_transaction = Transaction.new(
        ...
        client: Client.find_or_initialize_by name: row["client name"]
      )

      # Update the merchant address, if necessary:
      if a_transaction.merchant.address != row["merchant_address"]
        a_transaction.merchant.address = row["merchant_address"]
      end
